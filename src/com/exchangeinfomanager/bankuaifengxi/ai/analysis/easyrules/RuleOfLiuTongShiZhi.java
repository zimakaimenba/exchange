...
public class RuleOfLiuTongShiZhi //extends BasicRule //extends RuleOfNodeXdataBasic
{
    		...
    		@Fact("evacond") BanKuaiAndGeGuMatchingConditions evacond ) 
	{
	    ...
	    Double curltsz = null;
	    try {curltsz = ((StockNodesXPeriodData)nodexdata).getSpecificTimeLiuTongShiZhi(requireddate);
	    ...
	    if(curltsz == null) //有时候周一网易的数据还没有导入，导致没有流通市值数据，先用上一周的数据顶一下，毕竟不会相差太大
	    	...
	    	curltsz = ((StockNodesXPeriodData)nodexdata).getSpecificTimeLiuTongShiZhi(requireddate);
	    try {
